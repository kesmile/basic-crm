...
import { deleteProject, fetchProjects, setNameFilter } from '../features/projects/projectSlice';

const Projects = () => {
  const dispatch: AppDispatch = useDispatch();
  const { projects, status, total, nameFilter } = useSelector((state: RootState) => state.projects);
  const [open, setOpen] = useState(false);
  const [selectedProjectId, setSelectedProjectId] = useState<number | null>(null);
  const [page, setPage] = useState(1);
  const limit = 10;
  useEffect(() => {
    dispatch(fetchProjects({ page, limit, name: nameFilter }));
